...
- Because of that, the current product decision remains to STOP trying to ship the full desktop editor flow right now.
- The user manually deleted both previous macOS GitHub releases during this cleanup/reset stage.
- New desktop plan:
  - ship a recorder-only mac desktop `alpha`
  - app bundle / branding should be `LoomLess (alpha)`
  - user records normally
  - when recording stops, the recorded file should download/save directly
  - no editor flow in this alpha build
...
- Add a clear disclaimer in the app (`About`, settings, and if useful even the home screen) saying editing/export workflow is coming in future updates and this desktop build is an alpha, so bugs should be expected.
...
> "We're currently on the `tauri-desktop-app` branch — per the branch-scoping rules in AGENTS.md, this branch only touches `desktop-app/`. The change you're asking for lives in `website/`, which belongs on the `main` branch. Please switch with `git checkout main` and I'll pick it up there."
...
- Apple's TCC (camera/mic permission system) was designed around Developer ID signing ($99/yr Apple account). Paid signing gives the app a stable Team Identifier that's sealed into the binary, so TCC recognises "v2 of LoomLess" as the same entity as "v1 of LoomLess" and never re-prompts after the first grant
- Without a Developer ID we are stuck with ad-hoc signing (`signingIdentity: "-"`). Every rebuild produces a new CDHash; TCC has no Team Identifier to hang onto, so macOS may treat each rebuild as a fresh unknown app and re-issue prompts. This is an Apple/macOS design constraint, NOT a Tauri bug — Electron, native C++ apps, and anything else behave identically without Developer ID
- The reason OBS/Slack/VSCode never show this behaviour is that they are all signed with paid Developer IDs, not because they use a different UI framework
- For END USERS downloading a release DMG the experience is still ONE prompt on first launch and silent thereafter (they don't rebuild), provided steps 3 + 4 of the Build Process ran so both binaries share the `com.loomless.desktop` identifier. The double-prompt churn mainly affects DEV rebuilds where CDHash changes frequently
- Long-term, for professional distribution, get a Developer ID and sign+notarize. That is the only way to also eliminate the Gatekeeper "LoomLess cannot be opened because Apple cannot check it" warning that users hit on first open

**mainBinaryName (why the prompt says "LoomLess" and not "desktop-app"):**
- In Tauri v1, `productName` auto-renamed the main executable inside the bundle. In Tauri v2 you MUST also set `mainBinaryName` explicitly — otherwise the binary inside `Contents/MacOS/` keeps the Cargo package name (e.g. `desktop-app`)
- Ad-hoc signed apps have `Info.plist=not bound`, so macOS TCC can't trust `CFBundleDisplayName` and falls back to the executable filename for the prompt label. With `mainBinaryName = "LoomLess"`, the binary is named `LoomLess`, so the prompt reads "LoomLess would like to access the Camera" instead of "desktop-app would like to access the Camera"
- Our `tauri.conf.json` sets `productName: "LoomLess (alpha)"` and `mainBinaryName: "LoomLess"`. Do NOT remove either
...
- Do NOT wrap the sidecar in its own nested `.app` bundle with a separate `CFBundleIdentifier` — that also splits the TCC identity and causes a second prompt (previous regression)

**Build Process (ALWAYS run every step in order — skipping step 3 or step 4 reintroduces the double-prompt bug):**

All commands below assume the current working directory is `desktop-app/`. Run `cd desktop-app` first if you are at the repo root.

**Step 1 — Compile the Swift sidecar**
```bash
bash sidecar/build.sh
```
What this does: compiles `sidecar/camera-overlay/main.swift` with `swiftc` and writes the raw Mach-O binary to `src-tauri/binaries/camera-overlay-aarch64-apple-darwin` (plus an x86_64 variant on Intel hosts). Also cleans up any legacy `CameraOverlay.app` nested-bundle artifacts.

Verify:
```bash
file src-tauri/binaries/camera-overlay-aarch64-apple-darwin
# Expected: "... Mach-O 64-bit executable arm64"
```
If it says `shell script` or the file is missing, the build failed — do NOT proceed.

**Step 2 — Build the Tauri app**
```bash
npm run tauri build
```
What this does: compiles the Rust code, copies the frontend from `src/` and the sidecar from `src-tauri/binaries/camera-overlay-*` into `src-tauri/target/release/bundle/macos/LoomLess (alpha).app/Contents/MacOS/`, and ad-hoc signs the bundle (because `tauri.conf.json` has `bundle.macOS.signingIdentity = "-"`).

Known limitation: Tauri's `--deep` signing pass does NOT always re-sign externalBins. After this step the sidecar usually still has its linker-signed identifier (e.g. `camera-overlay-<hash>`), which is why step 3 is mandatory.

**Step 3 — Re-sign the whole bundle (MANDATORY — fixes double-prompt)**

Order matters here. `codesign --deep` does NOT correctly re-sign standalone Mach-O binaries sitting next to the main executable inside `Contents/MacOS/` (it preserves their original embedded `Identifier`). And if you sign the sidecar first and then run `--deep` on the bundle, `--deep` walks back over the sidecar and restores its linker-signed identifier. So you MUST:

1. Re-sign the bundle first:
   ```bash
   codesign --force --deep --sign - "src-tauri/target/release/bundle/macos/LoomLess (alpha).app"
   ```
2. THEN explicitly re-sign the sidecar with the bundle's identifier (this must come AFTER the bundle signing, not before):
   ```bash
   codesign --force --sign - --identifier com.loomless.desktop \
     "src-tauri/target/release/bundle/macos/LoomLess (alpha).app/Contents/MacOS/camera-overlay"
   ```

Result: both binaries share the `com.loomless.desktop` TCC identity, so the user sees ONE camera prompt saying "LoomLess wants camera access".

**Step 4 — Verify the identifiers match (MANDATORY)**
```bash
codesign -dv "src-tauri/target/release/bundle/macos/LoomLess (alpha).app/Contents/MacOS/LoomLess" 2>&1 | grep Identifier
codesign -dv "src-tauri/target/release/bundle/macos/LoomLess (alpha).app/Contents/MacOS/camera-overlay" 2>&1 | grep Identifier
```
Both commands MUST print `Identifier=com.loomless.desktop`. If the sidecar still prints `Identifier=camera-overlay-...`, step 3 failed — re-run it with sudo or investigate why codesign rejected the sidecar.

**Step 5 — Reset stale TCC records (only needed when the signature changed since the last run — i.e. basically every rebuild during development)**
```bash
tccutil reset Camera com.loomless.desktop
tccutil reset Microphone com.loomless.desktop
```
What this does: removes old grant records for our bundle identifier so the next launch triggers a fresh, single prompt. Without this, macOS may still hold onto records tied to a previous CDHash and behave unpredictably (denying access even when System Settings shows the toggle as ON).

**Step 6 — Launch and verify**
```bash
open "src-tauri/target/release/bundle/macos/LoomLess (alpha).app"
```
Pick a camera mode (e.g. Screen + Cam). You should see EXACTLY ONE system prompt titled "LoomLess" would like to access the Camera. If you still see a second prompt labeled "desktop-app" or "camera-overlay", step 3 or step 5 was skipped or failed — go back and re-run them.

**If two prompts reappear after a rebuild:**
- Re-run step 3 (codesign) and step 4 (verification) — the identifiers MUST match
- Re-run step 5 (tccutil reset)
- Check `src-tauri/target/release/bundle/macos/LoomLess (alpha).app/Contents/MacOS/` contains only `desktop-app` and `camera-overlay` (no stray `CameraOverlay.app` nested bundle, no extra helper binaries)
- Check System Settings → Privacy & Security → Camera: if there is a stale `desktop-app` entry alongside `LoomLess (alpha).app`, select it and click `-` to remove it, then relaunch
